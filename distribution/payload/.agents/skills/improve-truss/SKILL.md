---
name: improve-truss
description: Run one explicitly authorized, evidence-backed improvement to a repository's agent guidance, tools, runbooks, or validation. Use only when the user invokes `$improve-truss` or explicitly asks to improve the Truss after observed reusable agent friction. Do not use for ordinary product changes, speculative cleanup, one unexplained agent mistake, or automatic post-task reflection.
---
...
Assign the correction to the Truss source, the consumer repository, the
