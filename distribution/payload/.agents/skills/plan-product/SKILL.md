...
`.truss/authority/decisions/`, and any input documents the user supplies. If
...
Write proposals into the existing owners — nowhere else. The draft files must
exist at their destinations before you ask for approval: a plan that lives
only in the conversation is lost when the session ends and fails the handback.
A draft section recording a public contract states that changes to it are
delivery-routed unless the owner waives that.

| Content | Destination |
| --- | --- |
| Goal, users, expected behavior, scope, acceptance criteria | `.truss/authority/product/<initiative>.md` |
| Lasting product or architecture choices | `.truss/authority/decisions/<decision>.md` (proposal status) |
| First implementation slice, owner, and validation intent | One plan in `.truss/authority/plans/active/`, only when the work needs durable memory |
...
- Approval covers the named content at that revision. Record accepted
  ...
  accepted, with the approval date and revision in the section itself.
- Content the user does not approve stays draft. Partial approval is normal.
- A material change to accepted content requires renewed approval. When the
  user authorizes such a change, update the acceptance stamp in the same edit
  — new date, new revision, and the authorization source — so no stamp names
...
- Every draft exists as a file at its destination path, not only in this
  conversation.
- No approval stamp names content that no longer exists; accepted content
  changed under new approval carries an updated stamp.
